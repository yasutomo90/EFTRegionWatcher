...
Compress-Archive -LiteralPath @((Join-Path $out 'EFTRegionWatcher.exe'), (Join-Path $out 'EFTRegionWatcher.Updater.exe'), (Join-Path $out 'README.md'), (Join-Path $out 'LICENSE'), (Join-Path $out 'LICENSE-NotoSansJP.txt'), (Join-Path $out 'SHA256SUMS.txt')) -DestinationPath (Join-Path $out 'EFTRegionWatcher-win-x64.zip') -Force
